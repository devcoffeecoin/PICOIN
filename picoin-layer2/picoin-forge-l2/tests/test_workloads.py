...
from picoin_forge_l2.worker.workloads import classify_text_payload, embed_text_payload, solve_workload, summarize_text_payload
...
def test_text_embedding_payload_is_deterministic():
    first = embed_text_payload({"text": "Picoin compute workers verify useful AI tasks.", "dimensions": 8})
    second = embed_text_payload({"text": "Picoin compute workers verify useful AI tasks.", "dimensions": 8})

    assert first == second
    assert first["dimensions"] == 8
    assert first["token_count"] > 0
    assert len(first["vector"]) == 8
    assert first["vector_hash"]


def test_workload_queue_verifies_text_embed_task(tmp_path, monkeypatch):
    monkeypatch.setattr(coordinator_main, "DEFAULT_COORDINATOR_STATE_DIR", str(tmp_path))
    client = TestClient(coordinator_main.api)
    registration = register_worker("PITEXTEMBED", tmp_path / "worker")
    assert client.post("/workers/register", json=registration.model_dump(mode="json")).status_code == 200
    ...
    created_response = client.post("/workloads", json={"task_type": "text_embed", "payload": payload})
    assert created_response.status_code == 200
    created = created_response.json()
