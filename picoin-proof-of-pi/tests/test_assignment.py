...
def test_expired_committed_competitive_task_commit_resumes_directly(tmp_path, monkeypatch) -> None:
    db_path = tmp_path / "assignment-expired-committed-commit-resume.sqlite3"
    monkeypatch.setattr("app.db.database.DATABASE_PATH", db_path)
    monkeypatch.setattr("app.core.settings.DATABASE_PATH", db_path)
    monkeypatch.setattr(mining_service, "MINING_TASK_MODE", "competitive_round")
    init_db(db_path)
    ...
    keypair = generate_keypair()
    miner = register_miner("resume-expired-committed-at-commit-miner", keypair["public_key"])
    task = create_next_task(miner["miner_id"])
    segment = calculate_pi_segment(task["range_start"], task["range_end"], task["algorithm"])
    result_hash = hash_result(segment, task["range_start"], task["range_end"], task["algorithm"])
    root = merkle_root(segment, task["range_start"])
    ...
    assert retry["message"] == "task already committed; reveal requested samples"
    with get_connection() as connection:
        row = connection.execute(
            "SELECT status, expires_at FROM tasks WHERE task_id = ?",
            (task["task_id"],),
        ).fetchone()
    assert row["status"] == "committed"
    assert row["expires_at"] != "2026-06-01T00:00:00+00:00"
